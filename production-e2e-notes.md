...
- Official FFmpeg.wasm documentation confirms `@ffmpeg/core` is single-threaded while `@ffmpeg/core-mt` is multi-threaded and needs additional worker support: https://ffmpegwasm.netlify.app/docs/overview/. The module worker in the installed package loads UMD with `importScripts` or ESM with dynamic `import`, so a Blob URL must point to the matching ESM core. Production deployment did not expose the application-set COOP/COEP headers even though preview did; the live site therefore reports `crossOriginIsolated: false` and no `SharedArrayBuffer`. A final ESM 0.12.9 compatibility retest is in progress before determining the production-safe media path.
- The ESM 0.12.9 matching-core retest succeeded in preview: the same 15.7 KB WAV completed and exposed `sample-converted.mp3` at 10.2 KB. This confirms the module worker must receive an ESM Blob core (not a UMD Blob or direct UMD URL). The fix is queued for the next publish, after which the production converter will be tested again in the non-isolated deployment context.
- After the ESM publish, production did receive the core JS and WASM successfully but held at 3%. The root cause was isolated to the emitted raw `worker?url` asset: it imported `./const.js` and `./errors.js`, which resolve under `/assets/` to the app HTML fallback (HTTP 200, `text/html`) rather than JavaScript modules. The worker import is now `?worker&url`, so Vite bundles those dependencies. The completed build no longer contains those unresolved relative imports, and a real preview WAV conversion again completed to 10.2 KB MP3. The production re-test follows the next publish.

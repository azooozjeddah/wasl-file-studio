...
const mocks = vi.hoisted(() => ({ recognize: vi.fn(async () => ({ data: { text: "نص مستخرج / extracted text" } })), terminate: vi.fn(async () => undefined) }));

vi.mock("tesseract.js", () => ({ createWorker: vi.fn(async () => ({ recognize: mocks.recognize, terminate: mocks.terminate })) }));

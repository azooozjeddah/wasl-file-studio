...
import { countPdfFormFields, parsePageList } from "./pdf-engine";
...
describe("parsePageList", () => {
  it("keeps the browser document distinct from the PDF document while rendering pages", () => {
    ...
    expect(source).toContain("catch { image = await output.embedPng(await canvasToPng(file)); }");
  });
...
describe("flatten PDF", () => {
  it("detects real AcroForm fields and uses the rasterized rebuild path", async () => {
    ...
    const source = readFileSync(resolve(process.cwd(), "client/src/lib/pdf-engine.ts"), "utf8");
    expect(source).toContain("async function flattenPdfForm");
    expect(source).toContain("await page.render");
    expect(source).toContain('flatten: "rasterized-rebuild"');
  });

...
export async function extractOcr(files: File[], language: "ara" | "eng" | "ara+eng", report?: (fraction: number) => void, signal?: AbortSignal): Promise<LocalFileResult[]> {
  ...
  try { const recognizeWithFallback = async (input: Blob | File) => { let recognized = await worker.recognize(input); if (recognized.data.text.trim()) return recognized.data.text; await worker.setParameters({ tessedit_pageseg_mode: PSM.SINGLE_LINE }); recognized = await worker.recognize(input); await worker.setParameters({ tessedit_pageseg_mode: PSM.AUTO }); return recognized.data.text; }; const allText: string[] = []; for (let index = 0; index < files.length; index += 1) { if (signal?.aborted) throw new DOMException("OCR cancelled", "AbortError"); const file = files[index]; if (file.type === "application/pdf") { const images = await pdfToImages(file, "png", .9, amount => report?.((index + amount) / files.length * .55)); for (const image of images) { if (signal?.aborted) throw new DOMException("OCR cancelled", "AbortError"); allText.push(await recognizeWithFallback(image.blob)); } } else { allText.push(await recognizeWithFallback(file)); if (signal?.aborted) throw new DOMException("OCR cancelled", "AbortError"); } report?.((index + 1) / files.length); }

...
export async function alterPdf(slug: string, file: File, options: PdfOptions, report?: (fraction: number) => void): Promise<LocalFileResult[]> {
  ...
  if (slug === "watermark-pdf") { const font = await source.embedFont(StandardFonts.HelveticaBold); pages.forEach((page, index) => { if (!selected.includes(index)) return; const { width, height } = page.getSize(); const text = options.watermark?.trim() || "WASL"; const size = Math.max(10, Math.min(120, options.watermarkSize || Math.min(width, height) / 10)); const color = /^#[0-9a-fA-F]{6}$/.test(options.watermarkColor || "") ? options.watermarkColor! : "#7352f4"; const red = parseInt(color.slice(1, 3), 16) / 255; const green = parseInt(color.slice(3, 5), 16) / 255; const blue = parseInt(color.slice(5, 7), 16) / 255; const margin = 28; const textWidth = font.widthOfTextAtSize(text, size); const position = options.watermarkPosition || "center"; const x = position.includes("right") ? width - textWidth - margin : position.includes("left") ? margin : (width - textWidth) / 2; const y = position.includes("top") ? height - size - margin : position.includes("bottom") ? margin : (height - size) / 2; page.drawText(text, { x, y, size, font, color: rgb(red, green, blue), opacity: Math.max(.05, Math.min(.95, options.watermarkOpacity ?? .24)), rotate: position === "center" ? degrees(38) : degrees(0) }); }); }

...
import { findTool, isOfficeTool, toolIconFor } from "@/lib/tools";
...
export default function ToolPage() {
  ...
  const lifecycleStatus = availability.data?.lifecycleStatus || record?.lifecycleStatus || "beta";
  ...
  const serverProcessed = tool.processingMode === "server";
  const localAnswer = tool.local ? t("تعمل هذه الأداة محليًا داخل متصفحك ولا تُرفع ملفاتها من أجل التحويل.", "This tool runs locally in your browser and does not upload files for conversion.") : serverProcessed ? t("يُنقل الملف وكلمة المرور إلى الخادم أثناء الطلب فقط، دون تخزين دائم.", "The file and password are sent to the server for the request only, without persistent storage.") : t("يعرض الموقع بوضوح طريقة المعالجة قبل بدء الاستخدام.", "The site clearly explains processing mode before use.");
  const processAnswer = tool.local ? t("اختر الملفات، عدّل الإعدادات المتاحة، ثم نزّل النتيجة التي ينشئها المتصفح.", "Choose files, adjust available settings, then download the result generated by the browser.") : t("اختر ملفك، أدخل كلمة المرور، ثم سجّل الدخول لبدء المعالجة الخادمية وتنزيل النتيجة.", "Choose your file, enter the password, then sign in to process it on the server and download the result.");
  const faq = { "@context": "https://schema.org", "@type": "FAQPage", mainEntity: [{ "@type": "Question", name: t(`كيف تعمل أداة ${name}؟`, `How does ${name} work?`), acceptedAnswer: { "@type": "Answer", text: processAnswer } }, { "@type": "Question", name: t("هل الملف آمن؟", "Is the file safe?"), acceptedAnswer: { "@type": "Answer", text: localAnswer } }, { "@type": "Question", name: t("ما الصيغ المدعومة؟", "Which formats are supported?"), acceptedAnswer: { "@type": "Answer", text: formats.join(", ") } }] };

  return <PublicLayout><main className="tool-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faq) }}/><div className="container"><div className="tool-breadcrumb"><Link href="/">{t("الرئيسية", "Home")}</Link><span>/</span><Link href="/#tools">{t("الأدوات", "Tools")}</Link><span>/</span><b>{name}</b></div><section className="tool-hero"><span className={`tool-icon tool-icon-pro tool-tone-${visualTone}`}><Icon size={22}/></span><div><span className="section-eyebrow">{lifecycleStatus === "beta" ? t("Beta · تحت التحقق", "BETA · UNDER REVIEW") : `${tool.category.toUpperCase()} · ${tool.local ? t("محلي", "LOCAL") : serverProcessed ? t("خادمي", "SERVER") : t("قريبًا", "SOON")}`}</span><h1>{name}</h1><p>{description}</p>{lifecycleStatus === "beta" && <small className="mt-2 block text-sm text-muted-foreground">{t("هذه الأداة متاحة للتجربة المحدودة؛ راجع النتيجة قبل استخدامها في ملفات حساسة.", "This tool is available for limited testing; review the result before using it for sensitive files.")}</small>}</div></section>{ads.data?.map(slot => <div className="managed-ad-slot tool-ad-slot" key={slot.id}><span>مساحة إعلان مهيأة</span><b>{slot.label}</b><small>لا يُدرج محتوى إعلاني أو تتبع طرف ثالث في هذه المرحلة.</small></div>)}{isOfficeTool(tool.slug) ? <OfficeEncryptionWorkspace tool={tool}/> : tool.slug === "dxf-to-pdf" ? <DxfWorkspace/> : tool.category === "spreadsheet" ? <ExcelWorkspace tool={tool}/> : <ToolWorkspace tool={tool}/>}<section className="tool-explainer"><div><FileSearch size={20}/><h2>{t("كيف تعمل الأداة؟", "How this tool works")}</h2><p>{processAnswer}</p></div><div><LockKeyhole size={20}/><h2>{t("هل الملف آمن؟", "Is the file safe?")}</h2><p>{localAnswer}</p></div><div><CheckCircle2 size={20}/><h2>{t("الصيغ المدعومة", "Supported formats")}</h2><p>{formats.join(" · ")}</p></div></section></div></main></PublicLayout>;
}

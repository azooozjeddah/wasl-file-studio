...
export default function ToolPage() {
  ...
  const isOfficeEncryptionTool = ["protect-word", "unlock-word", "protect-excel", "unlock-excel", "protect-powerpoint", "unlock-powerpoint"].includes(tool?.slug || "");
  ...
  if (!disabled && catalogHasEntries && !record && !availability.data && !isOfficeEncryptionTool) return <NotFound/>;
  ...
  if (isOfficeEncryptionTool) return <PublicLayout><main className="tool-page"><div className="container"><section className="tool-hero"><span className={`tool-icon tool-icon-pro tool-tone-${visualTone}`}><Icon size={22}/></span><div><span className="section-eyebrow">{t("خدمة داخلية", "INTERNAL SERVICE")}</span><h1>{name}</h1><p>{description}</p></div></section><OfficeEncryptionWorkspace tool={tool}/></div></main></PublicLayout>;

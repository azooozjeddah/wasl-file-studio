...
export default function ToolPage() {
  ...
  const disabled = Boolean(availability.data && (!availability.data.isActive || availability.data.isAllowed === false));
  ...
  return <PublicLayout><main className="tool-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faq) }}/><div className="container"><div className="tool-breadcrumb"><Link href="/">{t("الرئيسية", "Home")}</Link><span>/</span><Link href="/#tools">{t("الأدوات", "Tools")}</Link><span>/</span><b>{name}</b></div><section className="tool-hero"><span className={`tool-icon tool-icon-pro tool-tone-${visualTone}`}><Icon size={22}/></span><div><span className="section-eyebrow">{tool.category.toUpperCase()} · {tool.local ? t("محلي", "LOCAL") : t("قريبًا", "SOON")}</span><h1>{name}</h1><p>{description}</p></div></section>{ads.data?.map(slot => <div className="managed-ad-slot tool-ad-slot" key={slot.id}><span>مساحة إعلان مهيأة</span><b>{slot.label}</b><small>لا يُدرج محتوى إعلاني أو تتبع طرف ثالث في هذه المرحلة.</small></div>)}{tool.category === "spreadsheet" ? <ExcelWorkspace tool={tool}/> : <ToolWorkspace tool={tool}/>}<section className="tool-explainer"><div><FileSearch size={20}/><h2>{t("كيف تعمل الأداة؟", "How this tool works")}</h2><p>{t("اختر الملف، اضبط الإعدادات، ثم ابدأ المعالجة. النتيجة تُجهز داخل المتصفح لتتمكن من تنزيلها فورًا.", "Choose your file, configure the settings, then process. Your result is prepared inside the browser for immediate download.")}</p></div><div><LockKeyhole size={20}/><h2>{t("هل الملف آمن؟", "Is the file safe?")}</h2><p>{localAnswer}</p></div><div><CheckCircle2 size={20}/><h2>{t("الصيغ المدعومة", "Supported formats")}</h2><p>{formats.join(" · ")}</p></div></section></div></main></PublicLayout>;
}

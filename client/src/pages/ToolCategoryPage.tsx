...
export default function ToolCategoryPage() {
  ...
  const catalogTools = catalog.data ? catalog.data : toolDefinitions;
  const activeSlugs = new Set(catalogTools.map((tool: { slug: string }) => tool.slug));
  const lifecycleBySlug = new Map(catalogTools.map((tool: { slug: string; lifecycleStatus?: "ready" | "beta" | "maintenance" | "disabled" }) => [tool.slug, tool.lifecycleStatus]));

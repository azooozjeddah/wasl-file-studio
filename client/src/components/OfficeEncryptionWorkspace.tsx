...
const mimeFor = (family: string) => family === "word"
...
export default function OfficeEncryptionWorkspace({ tool }: { tool: InternalOfficeToolDefinition }) {
  ...
  const selectFile = (next?: File) => {
    ...
    if (next.type && next.type !== expectedMime && next.type !== "application/octet-stream") {
      return setError("نوع الملف لا يطابق صيغة Office المطلوبة.");
    }
    setFile(next);
  };

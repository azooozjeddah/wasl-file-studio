...
- [x] أداة واحدة فقط: فحص PDF إلى Word (`pdf-to-word`) بملف حقيقي وتحديد سبب عدم موثوقية الناتج قبل أي إصلاح أو تغيير حالة.
- [x] أداة واحدة فقط: إصلاح PDF إلى Word لمسار PDF النصي فقط، مع رفض واضح للملفات الممسوحة واختبار DOCX/TXT والتنزيل والواجهة وProduction قبل Ready.
- [x] PDF إلى Word فقط: اختبار PDF حقيقي متعدد الصفحات والتأكد من حفظ كل الصفحات والنصوص في DOCX الناتج.
- [x] PDF إلى Word فقط: اختبار PDF عربي حقيقي والتحقق من قابلية قراءة النص العربي داخل DOCX والتنزيل من Production والجوال.
- [x] PDF إلى Word فقط: التحقق من بقاء QR Ready وعاملة، ثم تثبيت اعتماد PDF إلى Word النهائي في checkpoint وGitHub دون لمس أدوات أخرى.
- [x] PDF إلى Word فقط: منع إنشاء DOCX غير مقروء عندما يعيد PDF محارف مشوهة بلا ترميز Unicode عربي، ثم إعادة اختبار العربية والمتعدد الصفحات.
- [x] PDF إلى Word فقط: عزل فشل العربية بين طبقة Unicode في PDF وPDF.js وRTL/shaping وDOCX، مع إبقاء الأداة Maintenance ودون OCR بديل.
- [x] PDF إلى Word فقط: اختبار تشخيصي مستقل للنص العربي داخل DOCX لعزل RTL/shaping عن طبقة استخراج PDF دون اعتماد الملف الاصطناعي كقبول نهائي.
- [x] PDF إلى Word فقط: اختبار PDF عربي بسيط وPDF عربي متعدد الصفحات وفحص DOCX الناتج فعليًا محليًا وعلى Production والجوال.
- [x] PDF إلى Word فقط: إصلاح المسار الحالي فقط إذا أثبت التشخيص إمكانه، ثم إعادة قبول جميع حالات اللغة والصفحات والتنزيل قبل أي Ready.
- [x] PDF إلى Word فقط: قبول Production لملف عربي Unicode متعدد الصفحات وفحص DOCX المفتوح والنص وRTL وفواصل الصفحات قبل أي تغيير حالة.
- [x] PDF إلى Word فقط: اختبار الجوال لنفس ملف القبول والتحقق من بقاء QR جاهزة، ثم اعتماد Ready وتوثيق قيد PDF بلا Unicode mapping إن نجحت جميع الشروط.
- [x] PDF إلى Word فقط: توضيح قيد Unicode mapping في الوصف العام للأداة دون تغيير أي تعريف أو واجهة لأداة أخرى.
- [x] PDF إلى Word فقط: اختيار PDF عربي حقيقي متعدد الصفحات والتحقق مسبقًا من أن طبقته النصية Unicode سليمة وقابلة للاستخراج.
- [x] PDF إلى Word فقط: اختبار الرفع والتحويل والتنزيل من Production وفتح DOCX لفحص RTL والنص والصفحات وعدم الفقد أو التكرار.
- [x] PDF إلى Word فقط: إعادة اختبار المسار على الجوال والتحقق من QR دون تعديلها، ثم تثبيت Ready أو إبقاء Maintenance وفق النتيجة.
- [x] PDF إلى Word فقط: إضافة اتجاه RTL صحيح إلى فقرات DOCX العربية ثم إعادة قبول التقرير العربي الرسمي متعدد الصفحات من Production والجوال.
- [x] PDF إلى Word فقط: اختيار PDF عربي رسمي أطول متعدد الصفحات مع Unicode قابل للاستخراج وتوثيق خصائصه قبل رفعه.
- [x] PDF إلى Word فقط: رفعه وتحويله وتنزيله من Production ثم فتح DOCX للتحقق من RTL والنص والصفحات وعدم الفقد أو التكرار.
- [x] PDF إلى Word فقط: اختبار النتيجة الطويلة على الجوال وتوثيق commit/checkpoint وسبب المشكلة والحل، دون أي تعديل QR أو أدوات أخرى.
- [x] PDF إلى Word فقط: تشغيل مسار التحويل مباشرة للتقرير العربي الرسمي الطويل عند تعذر TLS، ثم فحص DOCX وإثبات النص والصفحات وRTL قبل تأكيد Ready.
- [x] Baseline PDF إلى Word: توثيق اختبار التقرير العربي الرسمي 236 صفحة/3.9MB ونتيجته وقيود Unicode ومرجع الرجوع.
- [x] Baseline PDF إلى Word: تثبيت checkpoint وcommit ووسم GitHub واضحين دون تغيير أدوات أو QR.
- [x] الأداة التالية: إعلان الاسم والحالة والسبب المعروف وخطة الاختبار فقط، دون بدء أي تعديل قبل توجيه المستخدم.
- [x] OCR فقط: حفظ Baseline قابل للرجوع ثم تحديد محرك OCR والمكتبات والتحميلات والعمال وسبب ضعف العربية دون تعديل أدوات أخرى.
- [x] OCR فقط: تجهيز عينات عربية وإنجليزية حقيقية واضحة وملف متعدد الصفحات مع النص المرجعي، وقياس CER/WER مقارنة بالنص الأصلي.
- [x] OCR فقط: اختبار الرفع والمعالجة والتنزيل من Production والجوال، ثم إصلاح المحرك الحالي أو بديله المحلي فقط إن أثبت القياس الحاجة. المسار العام والجوال بقيا محجوبين بوضوح في Maintenance بعد فشل معيار الدقة، لذلك لم يجر اعتماد تشغيل أو تنزيل للمستخدمين.
- [x] OCR فقط: تثبيت Ready فقط عند معيار دقة موثق وثابت، أو إبقاؤه Maintenance مع سبب دقيق ومرجع checkpoint/commit.
- [x] OCR فقط: تقييم محركات OCR محلية بديلة متوافقة مع العربية في مسار تجريبي معزول، دون لمس PDF→Word أو QR أو خط الأساس المستقر. قُيّم PaddleOCR V5/V6 وTesseract best وPaddleJS/TrOCR/Kraken؛ لم يثبت مرشح متصفح صالح.
- [x] OCR فقط: تشغيل مقارنة Before/After موحدة للعربية والعربية+الإنجليزية والجداول والصفحات المتعددة وصور الجوال باستخدام CER وF1. رُفض كل مرشح قبل الصفحات المتعددة لأنه أخفق أو لم يحسن النثر/الجدول؛ وثّقت النتائج الموحدة وعينة الهاتف في تقرير المقارنة.
- [x] OCR فقط: اعتماد مرشح محلي فقط إذا تحسن بوضوح وبصورة متكررة، ثم اختبار Production والجوال أو توثيق عدم وجود بديل مقبول بأمان. لم يوجد مرشح يحقق الشرط؛ بقي Production والجوال محجوبين بوضوح وOCR Maintenance.
- [x] OCR فقط: إصدار تقرير تقني يشرح المحرك الحالي وإعدادات العربية وسبب مؤشرات CER/F1 وخطة القياس قبل أي تعديل Production.
- [x] OCR فقط: إعادة تقييم أفضل بديل محلي في مختبر معزول وبنفس عينات النثر والجداول والصفحات المتعددة والجوال. أعيد تقييم PaddleOCR وTesseract best، واستُبعد EasyOCR المحلي بعد توقف التهيئة بالموارد قبل أي ناتج؛ لا مرشح اجتاز النثر والجدول لبلوغ صفحة متعددة.
- [x] OCR فقط: مقارنة Before/After وتحديد ما إذا كان المحرك البديل يحقق عتبة اعتماد متكررة قبل أي دمج أو تغيير حالة. لا مرشح تجاوز خط العربية `CER 9.00%` أو جدول `F1 67.37%` بصورة قابلة للتكرار، لذلك لا دمج ولا Production ولا تغيير حالة.
- [x] OCR فقط: تثبيت عينات Benchmark ومعيار CER/F1/RTL/ترتيب الأسطر/الأعمدة/الزمن مع checkpoint 919bbf08 كخط مرجعي غير قابل للتعديل.
- [x] OCR فقط: تشغيل Benchmark معزول لـTesseract baseline وKraken إن أمكن وأي مرشح ONNX عربي موثوق دون تعديل المشروع أو Production.
- [x] OCR فقط: إصدار جدول مقارنة Engine/CER/F1/RTL/Multi-column/Speed/Notes وقرار صريح بعدم الدمج أو ترشيح مثبت قبل أي تغيير لاحق.
- [x] OCR فقط: تحليل مواضع فقد الدقة بين التعرف والتقسيم والتخطيط والـRTL في عينات Benchmark الحالية من دون تعديل المشروع.
- [x] OCR فقط: تنفيذ ثلاث تجارب pipeline معزولة: معالجة صورية للنثر، تقسيم أعمدة RTL للنص، وتقسيم مناطق للجدول قبل Tesseract الحالي.
- [x] OCR فقط: مقارنة CER/F1/RTL/ترتيب الأسطر/الزمن مع Baseline 919bbf08 وتوثيق ما إذا حققت أي تجربة معيار نجاح مكرر قبل أي اقتراح دمج. لم تنجح أي تجربة؛ لا اقتراح دمج أو تغيير Production.
- [x] OCR فقط: تنفيذ تجربة واحدة معزولة لاكتشاف صفوف وخلايا صفحة الجدول، وحفظ إحداثياتها وتحديد ترتيب RTL قبل OCR.
- [x] OCR فقط: تشغيل Tesseract الحالي لكل خلية بـPSM مناسب وتجميع row → RTL columns → cell text داخل المختبر فقط.
- [x] OCR فقط: إصدار Benchmark للخلايا يقارن CER/F1/Precision/Recall/RTL order/row binding والزمن مع Baseline، ثم التوقف بقرار صريح من دون تغيير Production. فشل المعيار: F1 انخفض رغم تحسن CER؛ لا اقتراح دمج.
...
- [x] repair-pdf فقط: حفظ نقطة رجوع قبل تعديل مسار التصنيف والتحقق، مع الحفاظ على OCR وQR وPDF→Word دون تغيير.
...
- [x] extract-pdf-pages فقط: تدقيق التنفيذ الحالي في ToolWorkspace وpdf-engine وتحديد العيب وخطة الإصلاح قبل أي تعديل.
- [x] extract-pdf-pages فقط: حفظ نقطة رجوع ثم تنفيذ إصلاح محصور واختباره على صفحات متتالية وغير متتالية وPDF عربي وصوري متعدد الصفحات.
- [x] extract-pdf-pages فقط: فحص المخرجات والبناء والـdiff وحفظ المرحلة مع بقاء Lifecycle Disabled ومن دون نشر Production.

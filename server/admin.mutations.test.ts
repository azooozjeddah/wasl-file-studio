...
describe("admin mutations", () => {
  ...
  it("persists operational management inputs and writes audit rows", async () => {
    ...
    await expect(caller.admin.saveSettings({ siteName: "وصل", taglineAr: "", taglineEn: "", logoText: "وصل", logoUrl: "", accentColor: "#7157F8", metaTitle: "", metaDescription: "", supportEmail: "", defaultLocale: "ar", defaultTheme: "light", adsEnabled: false, analyticsEnabled: true, localMaxFileMb: 100, serverProcessingEnabled: false, serverMaxFileMb: 250, serverRateLimitPerMinute: 10, temporaryRetentionHours: 24, defaultProcessingMode: "local" })).resolves.toEqual({ success: true });
